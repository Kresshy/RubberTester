...
public:
	...
	//for this filter 
	Moving_average(int length, long default_data=0L);
	...
	*/
    ...
    long current() {return ca;}
	void set_current(const long val) {ca = val;}
	
	//add a new data point and return the filtered result.  filter() also
	//sets current
	long filter(long new_data);	
